...
- **Per-agent key status in export summary**: `agentvault setup export` output now includes a per-agent key status table showing `[vault key included]`, `[env key included]`, `[no key needed]`, or `[redacted]` for each exported agent.
